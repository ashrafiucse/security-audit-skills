#!/usr/bin/env python3
"""selftest_patterns.py - verify detection greps still match their fixtures.

Each rule: (regex, fixture path, line substring that MUST be matched by >=1 hit).
Guards against pattern rot: if a references/SKILL.md grep is edited and stops
catching its planted finding, CI fails here before recall silently degrades.
"""
import re
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
FIXTURES = ROOT / "evals" / "fixtures"

RULES = [
    # --- injection-flaws: new classes (node-vuln-app-2) ---
    ("nosql-operator-injection",
     r"(find|findOne|findOneAndUpdate|updateOne|deleteOne|aggregate)\(\s*\{\s*\.\.\.(req|ctx|event)\.|\{\s*\.\.\.(body|query|params)|find(One)?\(\s*\{[^}]*req\.(body|query)",
     "node-vuln-app-2/app.js", "findOne({ email: req.body.email"),
    ("prototype-pollution",
     r"(deepMerge|defaultsDeep|merge\(|extend\(|\.set\()\(.*req\.(body|query|params)|Object\.assign\(\s*\{\}\s*,\s*req\.(body|query)",
     "node-vuln-app-2/app.js", "deepMerge(featureFlags, req.body)"),
    ("open-redirect-js",
     r"res\.redirect\(",
     "node-vuln-app-2/app.js", "res.redirect(req.query.next"),
    ("header-trust-authz",
     r"req\.headers\[\s*['\"]x-",
     "node-vuln-app-2/app.js", "req.headers['x-user-id']"),
    ("websocket-authz",
     r"socket\.on\(",
     "node-vuln-app-2/app.js", "socket.on('orders:subscribe'"),
    ("upload-filename",
     r"originalname",
     "node-vuln-app-2/app.js", "cb(null, file.originalname)"),
    # --- injection-flaws: new classes (python-vuln-app-2) ---
    ("xxe-lxml",
     r"etree\.(parse|fromstring)",
     "python-vuln-app-2/app.py", "etree.fromstring(request.get_data())"),
    ("xxe-stdlib-import",
     r"xml\.etree",
     "python-vuln-app-2/app.py", "import xml.etree.ElementTree as ET"),
    ("open-redirect-py",
     r"redirect\(request",
     "python-vuln-app-2/app.py", "redirect(request.args.get"),
    # --- auth-review: existing core classes stay covered (rails fixture) ---
    ("idor-sink",
     r"req\.params\.id|findById\(",
     "node-vuln-app/app.js", "req.params.id"),
    # --- auth-review: route census (node-vuln-app-3) ---
    ("route-census",
     r"app\.(get|post|put|patch|delete|all)\(",
     "node-vuln-app-3/app.js", "app.get('/admin/users'"),
    # --- injection-flaws: second-order + builder + worker (node-vuln-app-3) ---
    ("stored-xss-read-path",
     r"<%-",
     "node-vuln-app-3/views/profile.ejs", "user.bio"),
    ("query-builder-raw",
     r"\.raw\(",
     "node-vuln-app-3/app.js", "knex.raw('SELECT * FROM products ORDER BY '"),
    ("queue-worker-exec",
     r"exec\(",
     "node-vuln-app-3/app.js", "exec(`convert"),
    # --- container-iac-security + config-hardening CI (iac-vuln-app) ---
    ("k8s-privileged",
     r"privileged:\s*true",
     "iac-vuln-app/deployment.yaml", "privileged: true"),
    ("k8s-run-as-root",
     r"runAsUser:\s*0",
     "iac-vuln-app/deployment.yaml", "runAsUser: 0"),
    ("tf-open-ingress",
     r"0\.0\.0\.0/0",
     "iac-vuln-app/main.tf", 'cidr_blocks = ["0.0.0.0/0"]'),
    ("tf-unencrypted-db",
     r"storage_encrypted\s*=\s*false",
     "iac-vuln-app/main.tf", "storage_encrypted = false"),
    ("ci-pull-request-target",
     r"pull_request_target",
     "iac-vuln-app/.github/workflows/deploy.yml", "pull_request_target:"),
    ("ci-untrusted-checkout",
     r"github\.event\.pull_request\.head\.sha",
     "iac-vuln-app/.github/workflows/deploy.yml", "ref: ${{"),
    ("ci-tag-pinned-action",
     r"uses:\s*\S+@v\d",
     "iac-vuln-app/.github/workflows/deploy.yml", "actions/checkout@v4"),
    # --- auth-review: mass assignment (node-vuln-app-3) ---
    ("mass-assignment",
     r"\.update\(\s*req\.body",
     "node-vuln-app-3/app.js", ".update(req.body)"),
    # --- container-iac: IMDSv1 allowed (iac-vuln-app) ---
    ("imds-v1-allowed",
     r"http_tokens\s*=\s*\"optional\"",
     "iac-vuln-app/main.tf", 'http_tokens   = "optional"'),
    # --- config-hardening: postMessage (spa-vuln-app) ---
    ("postmessage-listener",
     r"addEventListener\(\s*['\"]message",
     "spa-vuln-app/index.html", "addEventListener('message'"),
    # --- config-hardening: CI workflow script injection (iac-vuln-app) ---
    ("ci-script-injection",
     r"github\.event\.pull_request\.(title|body|head\.ref)",
     "iac-vuln-app/.github/workflows/deploy.yml", "PR title: ${{"),
    # --- llm-security (llm-vuln-app) ---
    ("llm-unsafe-deserialization",
     r"(pickle|torch)\.(load|loads)",
     "llm-vuln-app/app.py", "pickle.load(f)"),
    ("llm-repl-tool",
     r"PythonREPLTool|ShellTool",
     "llm-vuln-app/app.py", "PythonREPLTool()"),
    ("llm-prompt-concat",
     r"prompt\s*=\s*f['\"]",
     "llm-vuln-app/app.py", "{user_message}"),
    ("llm-anthropic-key",
     r"sk-ant-api03-[A-Za-z0-9_-]{20,}",
     "llm-vuln-app/app.py", "sk-ant-api03-"),
    # --- vuln-db library families (java-libs-vuln, rails-libs-vuln) ---
    ("fastjson-autotype",
     r"setAutoTypeSupport|autoTypeSupport",
     "java-libs-vuln/src/main/java/com/example/web/ApiController.java", "setAutoTypeSupport(true)"),
    ("text4shell-substitutor",
     r"StringSubstitutor",
     "java-libs-vuln/src/main/java/com/example/web/ApiController.java", "StringSubstitutor.createDefault()"),
    ("shiro-default-key",
     r"kPH\+bIxk5D2deZiIxcaaaA==",
     "java-libs-vuln/src/main/java/com/example/web/ApiController.java", "kPH+bIxk5D2deZiIxcaaaA=="),
    ("rails-render-file-params",
     r"render\s+file:\s*params",
     "rails-libs-vuln/app/controllers/demos_controller.rb", "render file: params[:path]"),
    # --- config-hardening: client/API hardening (web-hardening-vuln) ---
    ("weak-csp",
     r"unsafe-(inline|eval)",
     "web-hardening-vuln/index.html", "'unsafe-inline'"),
    ("third-party-script-no-sri",
     r"<script[^>]+src=[\"']https?://",
     "web-hardening-vuln/index.html", "cdn.example-fake.com/analytics.js"),
    ("api10-upstream-innerhtml",
     r"innerHTML",
     "web-hardening-vuln/index.html", "innerHTML = data.html"),
    ("cookie-no-prefix",
     r"res\.cookie\(\s*['\"](?!__Host-|__Secure-)[^'\"]*['\"],.*secure",
     "web-hardening-vuln/app.js", "res.cookie('session'"),
    ("crlf-location-header",
     r"setHeader\(\s*['\"]Location",
     "web-hardening-vuln/app.js", "req.query.name"),
    ("api4-unbounded-find",
     r"\.find\(\s*\{\s*\}\s*\)",
     "web-hardening-vuln/app.js", "db.items.find({})"),
    # --- crypto-review (crypto-vuln-app) ---
    ("crypto-md5-password",
     r"hashlib\.md5",
     "crypto-vuln-app/app.py", "hashlib.md5(password"),
    ("crypto-ecb-mode",
     r"MODE_ECB",
     "crypto-vuln-app/app.py", "AES.MODE_ECB"),
    ("crypto-des",
     r"DES\.new\(",
     "crypto-vuln-app/app.py", "DES.new(b\"8bytekey\""),
    ("crypto-bad-random",
     r"random\.(randint|random)\(",
     "crypto-vuln-app/app.py", "random.randint"),
    ("crypto-tls-off",
     r"verify\s*=\s*False",
     "crypto-vuln-app/app.py", "verify=False"),
    ("crypto-weak-kdf",
     r"count\s*=\s*[0-9]{1,4}\b",
     "crypto-vuln-app/app.py", "count=1000"),
    # --- mobile-security: component census by type (mobile-vuln-app) ---
    ("android-exported-service",
     r"<service[^>]*android:exported=\"true\"",
     "mobile-vuln-app/AndroidManifest.xml", 'SyncService'),
    ("android-exported-activity",
     r"<activity[^>]*android:exported=\"true\"",
     "mobile-vuln-app/AndroidManifest.xml", 'AdminActivity'),
    # --- flow-security (flow-vuln-app) ---
    ("flow-client-total",
     r"req\.(body|query)\.(total|amount|price)",
     "flow-vuln-app/app.js", "req.body.total"),
    ("flow-order-id-fetch",
     r"req\.(body|query)\.order_id",
     "flow-vuln-app/app.js", "req.body.order_id"),
    ("flow-post-boundary-put",
     r"app\.put\(\s*['\"]\S*orders",
     "flow-vuln-app/app.js", "app.put('/orders/:id'"),
    ("flow-float-money",
     r"price\s*\*",
     "flow-vuln-app/app.js", "i.price * i.qty"),
    # --- dependency-vulns: beyond-CVEs pack (dep-risk-vuln-app) ---
    ("dep-vm2-usage",
     r"require\(\s*['\"]vm2['\"]\s*\)|new VM\(",
     "dep-risk-vuln-app/app.js", "new VM({"),
    ("dep-lodash-merge-usage",
     r"\.merge\(\s*\w+,\s*req\.body",
     "dep-risk-vuln-app/app.js", "_.merge(config, req.body)"),
    ("dep-request-usage",
     r"require\(\s*['\"]request['\"]\s*\)",
     "dep-risk-vuln-app/app.js", "require('request')"),
    ("dep-compromised-version",
     r"ua-parser-js['\"]\s*:\s*['\"]0\.7\.29",
     "dep-risk-vuln-app/package.json", '0.7.29'),
    ("dep-vendored-old-jquery",
     r"jQuery JavaScript Library v1\.",
     "dep-risk-vuln-app/public/vendor/jquery-1.8.3.min.js", "v1.8.3"),
    # --- proactive gap round (gaps3-vuln-app) ---
    ("tenant-unscoped-read",
     r"res\.json\(db\.reports\)",
     "gaps3-vuln-app/app.js", "res.json(db.reports)"),
    ("hook-registration-url",
     r"hooks\.push\(.*url:\s*req\.body\.url",
     "gaps3-vuln-app/app.js", "url: req.body.url"),
    ("hook-delivery-fetch",
     r"fetch\(\s*h\.url",
     "gaps3-vuln-app/app.js", "fetch(h.url"),
    ("zip-extract-overwrite",
     r"extractAllTo\(.*true\)",
     "gaps3-vuln-app/app.js", "extractAllTo("),
    ("host-substring-allowlist",
     r"host\.includes\(h\)",
     "gaps3-vuln-app/app.js", "host.includes(h)"),
    # --- C/C++ native pack (c-vuln-app) ---
    ("c-format-string",
     r"(printf|fprintf|syslog)\s*\(\s*\w+\s*[,)]|fprintf\s*\(\s*\w+,\s*[a-z_]",
     "c-vuln-app/service.c", "fprintf(stderr, user_agent)"),
    ("c-strcpy",
     r"\b(strcpy|strcat|gets|sprintf)\s*\(",
     "c-vuln-app/service.c", "strcpy(name, input)"),
    ("c-system-cmd",
     r"\bsystem\s*\(",
     "c-vuln-app/service.c", "system(cmd)"),
    # --- course-platform-security (course-vuln-app) ---
    ("catalog-unfiltered",
     r"res\.json\(db\.courses\)",
     "course-vuln-app/app.js", "res.json(db.courses)"),
    ("preview-full-content",
     r"lessons:\s*course\.content",
     "course-vuln-app/app.js", "lessons: course.content"),
    ("self-enrollment",
     r"userId:\s*req\.body\.userId",
     "course-vuln-app/app.js", "req.body.userId"),
    ("cohort-from-query",
     r"cohortId:\s*req\.query\.cohort_id",
     "course-vuln-app/app.js", "req.query.cohort_id"),
    ("admin-no-role",
     r"app\.post\(\s*['\"]\S*admin",
     "course-vuln-app/app.js", "app.post('/api/admin/courses'"),
    ("course-moderation-admin-html-sink",
     r"res\.send\(\s*`[^`]*\$\{r\.body\}",
     "course-vuln-app/app.js", "${r.body}"),
    # --- laravel-security: moderation-queue XSS census (live miss 2026-09-23) ---
    ("blade-moderation-detail-raw-render",
     r"\{!!",
     "laravel-vuln-app/resources/views/course-edit/reviews/view.blade.php", "{!! nl2br($review->body)"),
    ("blade-census-react-noise",
     r"\{!!",
     "laravel-vuln-app/resources/js/AdminDashboard.tsx", "aria-invalid={!!errors.body}"),
    ("review-validation-string-only",
     r"'body'\s*=>\s*\[\s*'required',\s*'string'",
     "laravel-vuln-app/app/Http/Requests/CourseReviewRequest.php", "'required', 'string'"),
    # --- self-audit 2026-09-23: previously unguarded fixtures get guards ---
    ("django-marksafe-xss",
     r"mark_safe\s*\(",
     "django-vuln-app/myapp/views.py", "mark_safe(request.GET.get"),
    ("django-modelform-all-fields",
     r"fields\s*=\s*'__all__'",
     "django-vuln-app/myapp/forms.py", "'__all__'"),
    ("rails-raw-xss",
     r"<%=\s*raw\b|\.html_safe|<%==",
     "rails-vuln-app/app/views/users/show.html.erb", "<%= raw @user.bio"),
    ("rails-permit-bang",
     r"\.permit!",
     "rails-vuln-app/app/controllers/users_controller.rb", "permit!"),
    ("spring-utext-xss",
     r"th:utext",
     "spring-vuln-app/src/main/resources/templates/user.html", "th:utext"),
    ("spring-permit-all-admin",
     r"permitAll\(\)",
     "spring-vuln-app/src/main/java/com/example/config/SecurityConfig.java", "permitAll()"),
    ("graphql-passwordhash-leak",
     r"passwordHash",
     "graphql-vuln-app/server.js", "passwordHash: String!"),
    ("python-pickle-loads",
     r"pickle\.loads\(",
     "python-vuln-app/app.py", "pickle.loads(request.get_data())"),
    ("python-shell-true-concat",
     r"shell\s*=\s*True",
     "python-vuln-app/app.py", "shell=True"),
    ("supply-typosquat",
     r"lodash2|reqeusts",
     "supply-chain-vuln/package.json", "lodash2"),
    ("infra-untagged-base",
     r"FROM\s+node:latest",
     "infra-vuln/Dockerfile", "FROM node:latest"),
    ("infra-docker-sock-mount",
     r"/var/run/docker\.sock",
     "infra-vuln/docker-compose.yml", "/var/run/docker.sock"),
    # --- sibling moderation-direction plants (class propagation, 2026-09-23) ---
    ("django-moderation-safe-filter",
     r"\|safe\b",
     "django-vuln-app/myapp/templates/reviews/moderation.html", "r.body|safe"),
    ("rails-moderation-raw",
     r"<%=\s*raw\b",
     "rails-vuln-app/app/views/reviews/_moderation.html.erb", "<%= raw @review.body"),
    ("spring-moderation-utext",
     r"th:utext",
     "spring-vuln-app/src/main/resources/templates/reviews/moderation.html", "th:utext=\"${review.body}"),
    ("blade-mail-raw-render",
     r"\{!!",
     "laravel-vuln-app/resources/views/mail/support-ticket.blade.php", "{!! $ticket->body !!}"),
    ("blade-quiz-grade-raw-render",
     r"\{!!",
     "laravel-vuln-app/resources/views/course-edit/quiz/grade.blade.php", "{!! nl2br($answer->text)"),
    ("redos-nested-quantifier",
     r"\(\w\+\)\+",
     "node-vuln-app-2/app.js", "(a+)"),
    # --- surface round (surface-vuln-app) ---
    ("saml-nonstrict",
     r"['\"]strict['\"]\s*:\s*False",
     "surface-vuln-app/saml_route.py", '"strict": False'),
    ("saml-unsigned",
     r"want(Response|Assertions)Signed['\"]\s*:\s*False",
     "surface-vuln-app/saml_route.py", "wantResponseSigned"),
    ("k8s-escalate-verbs",
     r"verbs:\s*\[.*['\"](escalate|bind|impersonate)['\"]",
     "surface-vuln-app/clusterrole.yaml", '"escalate", "bind"'),
    ("k8s-wildcard-rbac",
     r'(resources|apiGroups|verbs):\s*\[\s*"\*"\s*\]',
     "surface-vuln-app/clusterrole.yaml", 'resources: ["*"]'),
    ("serverless-wildcard-iam",
     r"Action:\s*['\"]\*['\"]",
     "surface-vuln-app/serverless.yml", 'Action: "*"'),
    ("serverless-authorizer-none",
     r"authorizer:\s*none",
     "surface-vuln-app/serverless.yml", "authorizer: none"),
    ("devart-vscode-token",
     r"GITHUB_TOKEN['\"]?\s*[:=]",
     "surface-vuln-app/.vscode/launch.json", 'GITHUB_TOKEN'),
    ("devart-postman-bearer",
     r"Bearer eyJ[A-Za-z0-9_.-]+",
     "surface-vuln-app/shop-api.postman_collection.json", 'Bearer eyJ'),
    # --- dotnet-security (dotnet-vuln-app, help-wanted #1 2026-09-23) ---
    ("dotnet-connstr-secret",
     r"Password=",
     "dotnet-vuln-app/appsettings.json", "Password=Fake4EvalsDoNotUse"),
    ("dotnet-allow-anonymous",
     r"\[AllowAnonymous\]",
     "dotnet-vuln-app/Controllers/ReportsController.cs", "[AllowAnonymous]"),
    ("dotnet-open-redirect",
     r"Redirect\(\s*Request\.Query",
     "dotnet-vuln-app/Controllers/ReportsController.cs", 'return Redirect(Request.Query["next"])'),
    ("dotnet-process-concat",
     r"ProcessStartInfo\([^)]*\+",
     "dotnet-vuln-app/Controllers/ReportsController.cs", 'pdf2txt " + file'),
    ("dotnet-mass-assign-role",
     r"user\.Role\s*=\s*model\.Role",
     "dotnet-vuln-app/Controllers/ReportsController.cs", "user.Role = model.Role"),
    ("dotnet-fromsqlraw",
     r"FromSqlRaw\(\$",
     "dotnet-vuln-app/Data/ReportRepository.cs", 'FromSqlRaw($"SELECT'),
    ("dotnet-dapper-concat",
     r"\.Query\(\s*\"[^\"]*\"\s*\+",
     "dotnet-vuln-app/Data/ReportRepository.cs", "Reports WHERE Title = '\" + title"),
    ("dotnet-binaryformatter",
     r"new\s+BinaryFormatter",
     "dotnet-vuln-app/Utils/LegacyImporter.cs", "new BinaryFormatter()"),
    ("dotnet-xxe-dtd",
     r"DtdProcessing\.Parse",
     "dotnet-vuln-app/Utils/LegacyImporter.cs", "DtdProcessing.Parse"),
    ("dotnet-raw-xss",
     r"Html\.Raw\(",
     "dotnet-vuln-app/Views/Reports/View.cshtml", "@Html.Raw(Model.Body)"),
    ("dotnet-machinekey-literal",
     r"validationKey=\"[0-9A-Fa-f]{20,}",
     "dotnet-vuln-app/web.config", 'validationKey="A1B2C3'),
    ("dotnet-cors-credentials",
     r"AllowAnyOrigin\(\)",
     "dotnet-vuln-app/Program.cs", ".AllowAnyOrigin()"),
    ("dotnet-typenamehandling",
     r"TypeNameHandling\.(All|Auto|Objects)",
     "dotnet-vuln-app/Program.cs", "TypeNameHandling.All"),
    ("dotnet-dev-exception-page",
     r"UseDeveloperExceptionPage\(",
     "dotnet-vuln-app/Program.cs", "app.UseDeveloperExceptionPage()"),
    ("dotnet-telerik-version",
     r"Include=\"Telerik[\w.]*\"\s+Version=\"(20[01][0-9]|2020)\.",
     "dotnet-vuln-app/dotnet-vuln-app.csproj", 'Include="Telerik.Web.UI"'),
    # --- auth-review: LDAP pack (surface-vuln-app, help-wanted #1a) ---
    ("ldap-anon-bind-py",
     r"authentication\s*=\s*ANONYMOUS|import.*ANONYMOUS",
     "surface-vuln-app/ldap_route.py", "authentication=ANONYMOUS"),
    ("ldap-dn-concat-py",
     r'"uid="\s*\+\s*uid',
     "surface-vuln-app/ldap_route.py", '"uid=" + uid + "," + BASE_DN'),
    ("ldap-filter-fstring",
     r"\(sAMAccountName=\{name\}",
     "surface-vuln-app/ldap_route.py", "(sAMAccountName={name})"),
    ("ldap-anon-bind-java",
     r'SECURITY_AUTHENTICATION"?,?\s*"?none',
     "surface-vuln-app/LdapAuth.java", '"none"'),
    ("ldap-dn-concat-java",
     r'"uid="\s*\+\s*uid',
     "surface-vuln-app/LdapAuth.java", '"uid=" + uid + ",ou=people'),
    # --- config-hardening: WebAssembly pack (spa-vuln-app, help-wanted #1b) ---
    ("wasm-user-url-fetch",
     r"fetch\(\s*url\s*\)",
     "spa-vuln-app/index.html", "await fetch(url)"),
    ("wasm-instantiate",
     r"WebAssembly\.instantiateStreaming\(",
     "spa-vuln-app/index.html", "WebAssembly.instantiateStreaming(res)"),
    # --- secrets-detection: ADO.NET connection-string pattern (dotnet-vuln-app) ---
    ("secrets-adonet-connstr",
     r'(Server|Data Source|SERVER|DATA SOURCE)=[^;]{2,};[^"]*[Pp]assword=',
     "dotnet-vuln-app/appsettings.json", "Password=Fake4EvalsDoNotUse"),
    # --- flutter-security (flutter-security-vuln-app) ---
    ("flutter-storage-prefs",
     r"SharedPreferences\.getInstance",
     "flutter-security-vuln-app/lib/storage.dart", "final prefs = await SharedPreferences.getInstance"),
    ("flutter-storage-file-token",
     r"writeAsString\(",
     "flutter-security-vuln-app/lib/storage.dart", "await tokenFile.writeAsString"),
    ("flutter-storage-sqflite-token",
     r"db\.insert\(",
     "flutter-security-vuln-app/lib/storage.dart", "db.insert("),
    ("flutter-badcert-global",
     r"badCertificateCallback",
     "flutter-security-vuln-app/lib/main.dart", "badCertificateCallback"),
    ("flutter-badcert-dio",
     r"onHttpClientCreate",
     "flutter-security-vuln-app/lib/network.dart", "onHttpClientCreate"),
    ("flutter-cleartext",
     r"http://",
     "flutter-security-vuln-app/lib/network.dart", "api.example-fake.com"),
    ("flutter-webview-js",
     r"JavascriptMode\.unrestricted",
     "flutter-security-vuln-app/lib/webview.dart", "JavascriptMode.unrestricted"),
    ("flutter-webview-channel",
     r"addJavaScriptChannel\(",
     "flutter-security-vuln-app/lib/webview.dart", "addJavaScriptChannel("),
    ("flutter-deeplink-route",
     r"pushNamed\(",
     "flutter-security-vuln-app/lib/deep_link.dart", "pushNamed(uri.path"),
    ("flutter-channel-method-name",
     r"invokeMethod\(action\)",
     "flutter-security-vuln-app/lib/channel.dart", "invokeMethod(action)"),
    ("flutter-channel-inbound-args",
     r"call\.arguments\[",
     "flutter-security-vuln-app/lib/channel.dart", "call.arguments["),
    ("flutter-dart-define-secret",
     r"--dart-define=\w+=",
     "flutter-security-vuln-app/scripts/build_prod.sh", "dart-define=API_KEY"),
    ("flutter-insecure-random",
     r"Random\(\)",
     "flutter-security-vuln-app/lib/crypto_util.dart", "final randomGen = Random"),
    ("flutter-md5-password",
     r"md5\.convert",
     "flutter-security-vuln-app/lib/crypto_util.dart", "md5.convert"),
    ("flutter-hardcoded-key",
     r"const String apiKey",
     "flutter-security-vuln-app/lib/network.dart", "static const String apiKey"),
    ("flutter-aws-key-plant",
     r"AKIA[0-9A-Z]{16}",
     "flutter-security-vuln-app/lib/network.dart", "AKIAIOSFODNN7"),
    # --- gaps4: adversarial drill vs injection-flaws (unknown sinks + split construction) ---
    ("gaps4-xss-insertadjacent",
     r"insertAdjacentHTML",
     "gaps4-vuln-app/app.js", "feed.insertAdjacentHTML"),
    ("gaps4-xss-outerhtml",
     r"outerHTML\s*=",
     "gaps4-vuln-app/app.js", "bioEl.outerHTML = user"),
    ("gaps4-sqli-indirect",
     r"(query|execute)\(\s*\w+\s*\+",
     "gaps4-vuln-app/app.js", "pool.query(base + req.params.ref"),
    ("gaps4-cmd-spawn-shell",
     r"spawn\([^)]*shell:\s*true",
     "gaps4-vuln-app/app.js", "shell: true"),
    ("gaps4-xss-triage-dompurify",
     r"innerHTML",
     "gaps4-vuln-app/triage-gaps4.js", "DOMPurify.sanitize"),
    # --- android-code-security (android-code-security-vuln-app) ---
    ("android-prefs-token",
     r"getSharedPreferences\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Storage.kt", "context.getSharedPreferences"),
    ("android-prefs-token-java",
     r"getSharedPreferences\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/LegacyStorage.java", "context.getSharedPreferences"),
    ("android-file-token",
     r"writeText\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Storage.kt", "tokenFile.writeText"),
    ("android-local-sqli-kt",
     r"execSQL\(.*\$",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Storage.kt", "db.execSQL(\"SELECT"),
    ("android-local-sqli-java",
     r"execSQL\(.*\+",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/LegacyStorage.java", "db.execSQL(\"SELECT"),
    ("android-trustmanager-empty",
     r"checkServerTrusted[^{]*\{\}",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Api.kt", "checkServerTrusted"),
    ("android-trustmanager-empty-java",
     r"checkServerTrusted[^{]*\{\}",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/LegacyApi.java", "checkServerTrusted"),
    ("android-hostnameverifier-kt",
     r"HostnameVerifier\s*\{[^}]*true",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Api.kt", "HostnameVerifier { _, _ -> true"),
    ("android-hostnameverifier-java",
     r"return\s+true;",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/LegacyApi.java", "return true;"),
    ("android-cleartext",
     r"http://",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Api.kt", "api.example-fake.com"),
    ("android-logcat-token",
     r"Log\.[dvi]\([^)]*[Tt]oken",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Api.kt", "token: $authToken"),
    ("android-js-interface",
     r"@JavascriptInterface",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/BillingWeb.kt", "@JavascriptInterface"),
    ("android-webview-var-url",
     r"loadUrl\((url|[a-z]+Url)",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/BillingWeb.kt", "webView.loadUrl(url)"),
    ("android-component-injection",
     r"getStringExtra\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/MainActivity.kt", "getStringExtra"),
    ("android-pendingintent-mutable",
     r"PendingIntent\.getActivity\(.*,\s*0\s*\)",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/MainActivity.kt", "PendingIntent.getActivity(this, 0, notify, 0)"),
    ("android-secretkeyspec",
     r"SecretKeySpec\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Crypto.kt", "SecretKeySpec(keyBytes"),
    ("android-ecb-default",
     r'Cipher\.getInstance\("AES"\)',
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Crypto.kt", "Cipher.getInstance"),
    ("android-md5",
     r'getInstance\("MD5"\)',
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Crypto.kt", "getInstance"),
    ("android-seeded-random",
     r"setSeed\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/Crypto.kt", "setSeed"),
    ("android-googleservices-key",
     r"AIza[0-9A-Za-z_\-]{35}",
     "android-code-security-vuln-app/app/google-services.json", "AIzaFakeForEvals"),
    # --- laravel-security Step 8: feature flags + email-blast amplification
    #     (incident 2026-09-23: trial-tenant mass-email abuse) ---
    ("laravel-flag-default-true",
     r"Feature::define\(\s*['\"][^'\"]+['\"]\s*,\s*true",
     "laravel-vuln-app/modules/marketing/src/Providers/MarketingServiceProvider.php",
     "Feature::define('import-leads', true)"),
    ("laravel-compose-auth-only",
     r"->middleware\(\['auth'\]\)",
     "laravel-vuln-app/modules/marketing/routes/web.php", "middleware(['auth'])"),
    ("laravel-compose-route-gated-safe",
     r"feature:send-email",
     "laravel-vuln-app/modules/marketing/routes/safe-web.php", "feature:send-email"),
    ("laravel-verification-link-get",
     r"Route::get\([^)]*send-verification",
     "laravel-vuln-app/modules/marketing/routes/web.php", "send-verification-link"),
    ("laravel-blast-unbounded",
     r"::query\(\)->get\(\)",
     "laravel-vuln-app/modules/marketing/src/Jobs/ProcessEmailBlast.php", "Lead::query()->get()"),
    ("laravel-blast-quota-safe",
     r"consumeForTenant|->limit\(",
     "laravel-vuln-app/modules/marketing/src/Jobs/SafeProcessEmailBlast.php", "consumeForTenant"),
    ("laravel-job-only-gate",
     r"Feature::active\('send-email'\)",
     "laravel-vuln-app/modules/marketing/src/Jobs/SendEmailToEmailBlastRecipient.php",
     "Feature::active('send-email')"),
    ("laravel-request-all-dto",
     r"\$request->all\(\)",
     "laravel-vuln-app/modules/marketing/src/Http/Controllers/LeadController.php",
     "new ComposeEmailDTO($request->all())"),
    ("laravel-pennant-db-store",
     r"'store'\s*=>\s*'database'",
     "laravel-vuln-app/config/pennant.php", "'database'"),
    ("laravel-deploy-purge-safe",
     r"pennant:purge",
     "laravel-vuln-app/deploy/safe-deploy.sh", "php artisan pennant:purge"),
    # --- generic layer: flag gating + F9 on a NON-Laravel fixture (gaps3, Node) ---
    ("gaps3-flag-default-true",
     r"(featureFlags|FEATURE_FLAGS|feature_flags)\s*=\s*\{[^}]*:\s*true",
     "gaps3-vuln-app/app.js", "'import-leads': true"),
    ("gaps3-compose-no-flag-middleware",
     r"app\.post\(\s*['\"][^'\"]*compose['\"]\s*,\s*\(",
     "gaps3-vuln-app/app.js", "'/admin/leads/email/compose'"),
    ("gaps3-blast-unbounded",
     r"findAll\(\s*\)",
     "gaps3-vuln-app/app.js", "db.leads.findAll()"),
    ("gaps3-f9-mail-loop",
     r"mailer\.sendMail",
     "gaps3-vuln-app/app.js", "lead.email"),
    ("gaps3-verification-get",
     r"app\.get\(\s*['\"][^'\"]*send-verification",
     "gaps3-vuln-app/app.js", "send-verification-link"),
]

# Raw-vulnerable-form patterns that must have ZERO hits in the safe counter-example files.
MUST_NOT_MATCH = [
    ("nosql-raw-body-query-safe",
     r"find(One)?\(\s*\{[^}]*req\.(body|query)",
     "node-vuln-app-2/safe-counterexamples.js"),
    ("merge-user-keys-safe",
     r"(deepMerge|merge\(|extend\(|\.set\()\(.*req\.(body|query|params)",
     "node-vuln-app-2/safe-counterexamples.js"),
    ("redirect-no-allowlist-safe",
     r"res\.redirect\(\s*req\.",
     "node-vuln-app-2/safe-counterexamples.js"),
    ("xxe-unsafe-parser-safe",
     r"(etree|ET)\.(parse|fromstring)\(\s*request",
     "python-vuln-app-2/safe_counterexamples.py"),
    ("redirect-raw-request-safe",
     r"redirect\(\s*request",
     "python-vuln-app-2/safe_counterexamples.py"),
    ("ejs-unescaped-read-safe",
     r"<%-",
     "node-vuln-app-3/safe-counterexamples.js"),
    ("unguarded-admin-route-safe",
     r"app\.get\(\s*['\"]\S*admin\S*['\"]\s*,\s*async",
     "node-vuln-app-3/safe-counterexamples.js"),
    ("k8s-privileged-safe",
     r"privileged:\s*true|runAsUser:\s*0",
     "iac-vuln-app/hardened-deployment.yaml"),
    ("mass-assignment-safe",
     r"\.update\(\s*req\.body",
     "node-vuln-app-3/safe-counterexamples.js"),
    ("postmessage-innerhtml-safe",
     r"innerHTML",
     "spa-vuln-app/safe-index.html"),
    ("llm-deserialization-safe",
     r"(pickle|torch)\.(load|loads)\(",
     "llm-vuln-app/safe_app.py"),
    ("llm-key-safe",
     r"sk-ant-api03-[A-Za-z0-9_-]{20,}",
     "llm-vuln-app/safe_app.py"),
    ("fastjson-autotype-safe",
     r"setAutoTypeSupport\(true\)|autoTypeSupport",
     "java-libs-vuln/src/main/java/com/example/web/SafeController.java"),
    ("shiro-default-key-safe",
     r"kPH\+bIxk5D2deZiIxcaaaA==|setCipherKey",
     "java-libs-vuln/src/main/java/com/example/web/SafeController.java"),
    ("weak-csp-safe",
     r"unsafe-(inline|eval)",
     "web-hardening-vuln/safe-index.html"),
    ("api10-innerhtml-safe",
     r"innerHTML",
     "web-hardening-vuln/safe-index.html"),
    ("crypto-md5-safe",
     r"hashlib\.md5",
     "crypto-vuln-app/safe_app.py"),
    ("crypto-ecb-tls-off-safe",
     r"MODE_ECB|verify\s*=\s*False",
     "crypto-vuln-app/safe_app.py"),
    ("flow-client-total-safe",
     r"req\.(body|query)\.(total|amount|price)",
     "flow-vuln-app/safe-flow.js"),
    ("flow-unguarded-assign-safe",
     r"Object\.assign\(order,\s*req\.body\)",
     "flow-vuln-app/safe-flow.js"),
    ("dep-vm2-request-safe",
     r"require\(\s*['\"](vm2|request)['\"]\s*\)",
     "dep-risk-vuln-app/safe-app.js"),
    ("dep-merge-reqbody-safe",
     r"\.merge\(\s*\w+,\s*req\.body",
     "dep-risk-vuln-app/safe-app.js"),
    ("dep-vendored-old-jquery-safe",
     r"jQuery JavaScript Library v1\.",
     "dep-risk-vuln-app/public/vendor/jquery-3.7.1.min.js"),
    ("tenant-unscoped-safe",
     r"res\.json\(db\.reports\)",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("zip-extract-overwrite-safe",
     r"extractAllTo\([^)]*true\)",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("host-substring-safe",
     r"host\.includes\(h\)",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("c-format-string-safe",
     r"fprintf\s*\(\s*\w+,\s*[a-z_]",
     "c-vuln-app/safe_service.c"),
    ("c-strcpy-safe",
     r"\b(strcpy|strcat|gets)\s*\(",
     "c-vuln-app/safe_service.c"),
    ("catalog-unfiltered-safe",
     r"res\.json\(db\.courses\)",
     "course-vuln-app/safe-platform.js"),
    ("self-enrollment-safe",
     r"userId:\s*req\.body\.userId",
     "course-vuln-app/safe-platform.js"),
    ("course-moderation-unescaped-safe",
     r"res\.send\(\s*`[^`]*\$\{r\.body\}",
     "course-vuln-app/safe-platform.js"),
    ("moderation-list-raw-echo-safe",
     r"\{!!",
     "laravel-vuln-app/resources/views/course-edit/reviews/index.blade.php"),
    ("moderation-detail-raw-body-safe",
     r"nl2br\(\s*\$review->body",
     "laravel-vuln-app/resources/views/course-edit/reviews/safe-detail-view.blade.php"),
    ("mail-escaped-safe",
     r"\{!!",
     "laravel-vuln-app/resources/views/mail/safe-support-ticket.blade.php"),
    ("quiz-grade-escaped-safe",
     r"nl2br\(\s*\$answer->text",
     "laravel-vuln-app/resources/views/course-edit/quiz/safe-grade.blade.php"),
    ("django-moderation-escaped-safe",
     r"\|safe\b",
     "django-vuln-app/myapp/templates/reviews/safe-moderation.html"),
    ("rails-moderation-escaped-safe",
     r"<%=\s*raw\b|\.html_safe",
     "rails-vuln-app/app/views/reviews/_safe_moderation.html.erb"),
    ("spring-moderation-escaped-safe",
     r"th:utext",
     "spring-vuln-app/src/main/resources/templates/reviews/safe-moderation.html"),
    # --- dotnet-security safe counter-examples ---
    ("dotnet-fromsqlraw-safe",
     r"FromSqlRaw\(",
     "dotnet-vuln-app/Data/SafeReportRepository.cs"),
    ("dotnet-dapper-raw-concat-safe",
     r"\.Query\(\s*\"[^\"]*\"\s*\+",
     "dotnet-vuln-app/Data/SafeReportRepository.cs"),
    ("dotnet-binaryformatter-safe",
     r"BinaryFormatter",
     "dotnet-vuln-app/Utils/SafeImporter.cs"),
    ("dotnet-xxe-dtd-safe",
     r"DtdProcessing\.Parse",
     "dotnet-vuln-app/Utils/SafeImporter.cs"),
    ("dotnet-raw-xss-safe",
     r"Html\.Raw\(",
     "dotnet-vuln-app/Views/Reports/SafeView.cshtml"),
    ("dotnet-machinekey-literal-safe",
     r"validationKey=\"[0-9A-Fa-f]{20,}",
     "dotnet-vuln-app/safe-web.config"),
    ("dotnet-allow-anon-safe",
     r"\[AllowAnonymous\]",
     "dotnet-vuln-app/Controllers/SafeReportsController.cs"),
    ("dotnet-role-assign-safe",
     r"\.Role\s*=\s*model\.Role",
     "dotnet-vuln-app/Controllers/SafeReportsController.cs"),
    ("dotnet-process-concat-safe",
     r"ProcessStartInfo\([^)]*\+",
     "dotnet-vuln-app/Controllers/SafeReportsController.cs"),
    # --- LDAP safe counter-examples ---
    ("ldap-dn-concat-py-safe",
     r'"uid="\s*\+\s*uid',
     "surface-vuln-app/safe-ldap_route.py"),
    ("ldap-dn-concat-java-safe",
     r'"uid="\s*\+\s*uid',
     "surface-vuln-app/SafeLdapAuth.java"),
    ("ldap-anon-bind-py-safe",
     r"ANONYMOUS",
     "surface-vuln-app/safe-ldap_route.py"),
    # --- WebAssembly safe counter-example ---
    ("wasm-user-url-fetch-safe",
     r"fetch\(\s*url\s*\)",
     "spa-vuln-app/safe-index.html"),
    ("laravel-deploy-purge-absent",
     r"pennant:purge",
     "laravel-vuln-app/deploy/deploy.sh"),
    ("gaps3-flag-default-true-safe",
     r"(featureFlags|FEATURE_FLAGS|feature_flags)\s*=\s*\{[^}]*:\s*true",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("gaps3-compose-no-flag-middleware-safe",
     r"app\.post\(\s*['\"][^'\"]*compose['\"]\s*,\s*\(",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("gaps3-blast-unbounded-safe",
     r"findAll\(\s*\)",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("gaps3-verification-get-safe",
     r"app\.get\(\s*['\"][^'\"]*send-verification",
     "gaps3-vuln-app/safe-gaps3.js"),
    ("laravel-flag-default-true-safe",
     r"Feature::define\(\s*['\"][^'\"]+['\"]\s*,\s*true",
     "laravel-vuln-app/modules/marketing/src/Providers/SafeMarketingServiceProvider.php"),
    ("laravel-compose-auth-only-safe",
     r"->middleware\(\['auth'\]\)",
     "laravel-vuln-app/modules/marketing/routes/safe-web.php"),
    ("laravel-verification-link-get-safe",
     r"Route::get\([^)]*send-verification",
     "laravel-vuln-app/modules/marketing/routes/safe-web.php"),
    ("laravel-blast-unbounded-safe",
     r"::query\(\)->get\(\)",
     "laravel-vuln-app/modules/marketing/src/Jobs/SafeProcessEmailBlast.php"),
    ("laravel-request-all-dto-safe",
     r"\$request->all\(\)",
     "laravel-vuln-app/modules/marketing/src/Http/Controllers/SafeLeadController.php"),
    # --- K8s wildcard RBAC safe counter-example (IBM-Concert class) ---
    ("k8s-wildcard-rbac-safe",
     r'(resources|apiGroups|verbs):\s*\[\s*"\*"\s*\]',
     "surface-vuln-app/hardened-clusterrole.yaml"),
    # --- flutter-security safe counter-examples (flutter-security-vuln-app) ---
    ("flutter-storage-prefs-safe",
     r"SharedPreferences\.getInstance",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-badcert-safe",
     r"badCertificateCallback|onBadCertificate",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-cleartext-safe",
     r"http://",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-webview-unrestricted-safe",
     r"JavascriptMode\.unrestricted",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-deeplink-pushnamed-safe",
     r"pushNamed\(",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-insecure-random-safe",
     r"Random\(\)",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-md5-safe",
     r"md5\.|sha1\.",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    ("flutter-dart-define-safe",
     r"dart-define",
     "flutter-security-vuln-app/lib/safe_counterexamples.dart"),
    # --- gaps4 safe counter-examples ---
    ("gaps4-unknown-sinks-safe",
     r"insertAdjacentHTML|outerHTML\s*=",
     "gaps4-vuln-app/safe-gaps4.js"),
    ("gaps4-sqli-indirect-safe",
     r"(query|execute)\(\s*\w+\s*\+",
     "gaps4-vuln-app/safe-gaps4.js"),
    ("gaps4-spawn-shell-safe",
     r"shell:\s*true",
     "gaps4-vuln-app/safe-gaps4.js"),
    # --- android-code-security safe counter-examples (SafeExamples.kt / SafeLegacyExamples.java) ---
    ("android-prefs-safe",
     r"getSharedPreferences\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-prefs-safe-java",
     r"getSharedPreferences\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeLegacyExamples.java"),
    ("android-trustmanager-safe",
     r"checkServerTrusted|HostnameVerifier",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-trustmanager-safe-java",
     r"checkServerTrusted|HostnameVerifier",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeLegacyExamples.java"),
    ("android-cleartext-safe",
     r"http://",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-cleartext-safe-java",
     r"http://",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeLegacyExamples.java"),
    ("android-js-interface-safe",
     r"@JavascriptInterface|addJavascriptInterface",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-webview-var-url-safe",
     r"loadUrl\((url|[a-z]+Url)",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-component-injection-safe",
     r"getStringExtra",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-pendingintent-mutable-safe",
     r"PendingIntent\.getActivity\(.*,\s*0\s*\)",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-secretkeyspec-safe",
     r"SecretKeySpec\(",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-ecb-safe",
     r'Cipher\.getInstance\("AES"\)',
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-md5-safe",
     r'getInstance\("(MD5|SHA-1)"\)',
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-seeded-random-safe",
     r"setSeed",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-sqli-interp-safe",
     r"execSQL\(.*(\$|\+)",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
    ("android-sqli-concat-safe-java",
     r"execSQL\(.*(\$|\+)",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeLegacyExamples.java"),
    ("android-logcat-token-safe",
     r"Log\.[dvi]\([^)]*[Tt]oken",
     "android-code-security-vuln-app/app/src/main/java/com/example/shop/SafeExamples.kt"),
]

# Multi-line rules: matched against the WHOLE FILE (python re, [\s\S] spans lines) —
# mirrors the bounded `rg -U` scans in injection-flaws.
CONTENT_RULES = [
    ("multiline-query-build",
     r"raw\(\s*`[\s\S]{0,300}(WHERE|ORDER BY)[\s\S]{0,300}\$\{",
     "node-vuln-app-3/app.js", "WHERE status = '${status}'"),
    ("toctou-check-then-claim",
     r"coupon\.used[\s\S]{0,200}updateOne",
     "node-vuln-app-2/app.js", "updateOne"),
]


def load(path):
    return path.read_text(encoding="utf-8").splitlines()


def run():
    failures = []
    for name, pattern, fixture, expected_sub in RULES:
        path = FIXTURES / fixture
        lines = load(path)
        rx = re.compile(pattern)
        hits = [(i, ln) for i, ln in enumerate(lines, 1) if rx.search(ln)]
        if not any(expected_sub in ln for _, ln in hits):
            failures.append(
                f"{name}: pattern no longer matches planted line in {fixture} "
                f"(expected substring {expected_sub!r}; {len(hits)} raw hits)")
    for name, pattern, fixture in MUST_NOT_MATCH:
        path = FIXTURES / fixture
        rx = re.compile(pattern)
        hits = [ln for ln in load(path) if rx.search(ln)]
        if hits:
            failures.append(
                f"{name}: vulnerable form matches {len(hits)} line(s) in {fixture}; "
                f"first: {hits[0].strip()[:100]}")
    for name, pattern, fixture, expected_sub in CONTENT_RULES:
        text = (FIXTURES / fixture).read_text(encoding="utf-8")
        rx = re.compile(pattern)
        if not any(expected_sub in m.group(0) for m in rx.finditer(text)):
            failures.append(
                f"{name}: multi-line pattern no longer matches {fixture} "
                f"(expected substring {expected_sub!r})")
    if failures:
        print("SELFTEST FAILURES:")
        for f in failures:
            print(f"  - {f}")
        return 1
    print(f"selftest_patterns: {len(RULES)} match rules + "
          f"{len(MUST_NOT_MATCH)} no-match + {len(CONTENT_RULES)} multi-line OK")
    return 0


if __name__ == "__main__":
    sys.exit(run())
