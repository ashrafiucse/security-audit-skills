...
| Native Android app code (Kotlin+Java) | good (first release) | `android-code-security`: app-code shapes in BOTH syntaxes — storage, local SQLi (execSQL/rawQuery interpolation), TLS bypass (TrustManager/HostnameVerifier), `@JavascriptInterface` bodies, deep-link component injection, PendingIntent mutability, Keystore/ECB/MD5/setSeed, logcat tokens, google-services.json — measured by `android-code-security-vuln-app` (22 rows, mixed-language tree) |
